      ...
      <div className="mt-4">
        <details className="mb-2 group">
          <summary className="cursor-pointer text-base font-medium mb-2 select-none flex items-center gap-2">
            ...
            <button
              className="px-2 py-1 text-xs rounded bg-gray-400 text-white hover:bg-gray-500 cursor-pointer"
          ...
          <thead className={isDark ? 'bg-zinc-900' : 'bg-blue-50'}>
            <tr>
              {visibleColumns.map((col) => (
                <th key={col} className={`px-3 py-2 text-left font-semibold sticky top-0 z-10 border-b select-none cursor-pointer items-center gap-1 ${isDark ? 'text-gray-100 bg-zinc-900 border-zinc-700' : 'text-gray-700 bg-blue-100 border-gray-200'}`}
                  onClick={() => {
                    setSortConfig(prev => {
